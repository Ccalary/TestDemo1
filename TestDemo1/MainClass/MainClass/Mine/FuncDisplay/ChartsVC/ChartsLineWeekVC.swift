...
@objc public class ChartsLineWeekVC: UIViewController {
        ...
        for item in jsonDataArray {
            if let dateTime = item.dateTime {
                if let power = item.generationPower {
                    let entry = ChartDataEntry(x: Double(dateTime), y: Double(arc4random()%1000))
//                    let entry = ChartDataEntry(x: Double(dateTime) - timeStamp, y: Double(10000))
                    ...
                    if let usePower = item.usePower {
                        let entry2 = ChartDataEntry(x: Double(dateTime), y: Double(arc4random()%1200))
                    ...
                    if let batteryPower = item.batteryPower {
                        let entry3 = ChartDataEntry(x: Double(dateTime), y: Double(arc4random()%1000))
                        entries3.append(entry3)
                        yMax = max(batteryPower, yMax)
                        yMin = min(batteryPower, yMin)
                    }
                }
                valueFormatterValues.append("\(item.timeStamp ?? "--")")
            }
        }
...
//        xAxis.valueFormatter =  TimeAxisValueFormatter()
        print("valueFormatterValues:\(valueFormatterValues)")
        xAxis.valueFormatter = IndexAxisValueFormatter(values: valueFormatterValues)
...
extension ChartsLineWeekVC: ChartViewDelegate {
