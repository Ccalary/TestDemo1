...
//  TestDemo1
//
//  Created by caohouhong on 2019/6/1.
...
- (void)viewDidLoad {
    ...
    _cityLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, ScreenWidth-20, 20)];
    _cityLabel.text = @"获取当前城市";
...
- (void)initLocationManager {
    if ([CLLocationManager locationServicesEnabled]) {//监测权限设置
        ...
        self.locationManager.desiredAccuracy = kCLLocationAccuracyThreeKilometers;//设置精度
    ...
    [geoCoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        if (placemarks.count > 0) {
            ...
            if (!city) {
                ...
                NSString *currentCity = placeMark.locality ;//获取当前城市
                NSLog(@"当前城市:%@",currentCity);
                _cityLabel.text = [NSString stringWithFormat:@"%@-%@-%@",placeMark.country,currentCity,placeMark.subLocality];
            }
